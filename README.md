...
건물 높이는 더 이상 플레이어가 직접 고르는 업그레이드가 아니며, 월드 쪽 값(0~3단계, 기본 0)으로만 존재합니다 — 다음 개발 단계의 난이도 시스템이 이 값을 올릴 예정입니다. 값이 오르면 그 시점 이후 새로 생성되는 청크만 건물·공중 장애물이 함께 높아지며, 이미 만들어진 청크는 그대로 유지됩니다. 공중 장애물은 64m마다 4개 이상(단계에 따라 증가), 각각 약 **2.6 × 2.2 × 1.4m**입니다. 연결된 장애물 뒤로 스윙해도 줄이 유지되며, 다른 장애물의 줄 가림과 몸 충돌은 위험합니다. 건물 벽은 몸으로 부딪혀도 안전합니다(벽 점프는 비활성화 상태). 도전 첫 128m와 연습 모드는 장애물 없이 시작합니다.
