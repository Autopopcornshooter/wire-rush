...
- **G:** 쌓인 업그레이드를 원하는 타이밍에 모두 소비(레벨업해도 바로 멈추지 않고 대기 stack에 쌓임).
...
높이 업그레이드는 현재·미래 건물을 **+10 / +20 / +30m** 높이고 공중 장애물도 **+8 / +16 / +24m** 올립니다. 공중 장애물은 64m마다 4개, 각각 약 **2.6 × 2.2 × 1.4m**입니다. 연결된 장애물 뒤로 스윙해도 줄이 유지되며, 다른 장애물의 줄 가림과 몸 충돌은 위험합니다. 건물 벽은 몸으로 부딪혀도 안전합니다(벽 점프는 비활성화 상태). 도전 첫 128m와 연습 모드는 장애물 없이 시작합니다.
