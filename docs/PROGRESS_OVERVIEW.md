...
| City Boss | "Police Interceptor"(그레이박스), 3200m 시작·1500m 진행(4700m) 시 클리어. HP/전투 없음 — PATH_BLOCK(한쪽 아파트 라인 전체가 1.5초간 접촉 즉사)→LASER_PLANE(양쪽 건물 라인을 가로지르는 넓은 면, 매번 랜덤 높이, telegraph 有, 방어구 1회 방어)→ROBOT_BARRAGE(장애물 로봇 3기 순차 직진 발사, 발사 시점 선행조준 후 비호밍, 자체 수명으로 정리) 고정 순서 반복. 항상 기본 사거리로 통과 가능한 route 보장. 클리어 시 후퇴·despawn |
