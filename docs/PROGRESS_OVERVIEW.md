# Wire Rush — 프로토타입 0.2.0

2026-09-14. 언어 설정과 건물 벽 직접 조준을 추가한 Windows 버전. 기존 자동 조준도 설정에서 선택할 수 있습니다.

## 실행 위치

- 프로젝트: `D:\GameProject\WireSwing\project.godot`
- 실행 파일: `builds/WireRush-0.2.0-Windows/WireRush.exe`
- 배포 ZIP: `builds/WireRush-0.2.0-Windows.zip`
...
- `tests/settings_aim.gd`: **35/35 통과**. 설정 저장·복원, 잘못된 값 복구, 한글 글꼴·문구, 설정 버튼·Esc, 정지 유지, 카메라 광선→서로 다른 벽과 높이 선택→실제 스윙, 재연결 후 이전 버튼 해제, 사거리·가림 검사, 잘못된 변경 시 줄 유지, 임시 앵커 정리, 원점 이동·청크 보호, 한 프레임 내 누름/해제, 자동 모드 복귀.
...
- 배포 EXE의 한국어 메뉴, 한국어 설정, 영어 설정, 한국어 직접 조준 플레이 화면을 캡처·검토. 네 프로세스 모두 **exit 0**, stderr 비어 있음. 직접 조준 캡처는 스크립트 입력을 사용한 렌더링 확인입니다.
- 배포 EXE SHA256: `44C677D1309FAFDBC4E700022324ED4D9234EFB849E2383858C9055E9B8B3FEC`.

자동 검사는 재현 가능한 기계적 동작 검사입니다. 여러 검사는 초기 위치와 속도를 지정하는 fixture를 사용합니다. 사용자의 자유 플레이, 체감 재미, 다른 GPU/PC에서의 실행까지 검증했다는 의미는 아닙니다.

메뉴와 플레이 캡처: `docs/screenshots/menu.png`, `docs/screenshots/gameplay.png`, `docs/screenshots/settings-ko.png`, `docs/screenshots/settings-en.png`. 로컬 상세 로그는 `validation/`에 생성되며 Git에서 제외합니다. 이전 `v0.1.0` 빌드는 로컬에서 그대로 보존합니다.
