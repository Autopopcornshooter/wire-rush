...
1. **핵심 체감 검증**: 사람의 입력으로 저공 스윙→슬라이드→재연결 5회 연속. 해제 0.15초·충격 4m/s·앵커 선택과 카메라 수치 조정.
2. **장애물 공정성**: 6종 청크 전체와 연결부를 기본 장비로 검사. 진입 속도별 보장 경로, 예고 가시성, 상부 차단물 난도, 구덩이 도입 여부.
3. **표현과 접근성**: 한국어 UI, 설정·음량, 선택 장비 전체 결과 표시, 조작 튜토리얼, 아트·애니메이션 교체.
