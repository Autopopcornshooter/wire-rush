# Wire Rush — 프로토타입 0.6.0

2026-09-14. E 두 줄 사출을 무적 없이 복구하고, 건물 간격/수직 차단벽 배치 선택과 자동 주행 51회 비교를 추가했습니다. [비교 결과](ROUTE_COMPARISON.md).

## 실행

- 프로젝트: `D:/GameProject/WireSwing/project.godot`
- 실행 파일: `builds/WireRush-0.6.0-Windows/WireRush.exe`
- 배포 ZIP: `builds/WireRush-0.6.0-Windows.zip`
...
| E 사출 | 전방 양쪽 건물 필요. 시작부터 사용. 28m/s, 퍽 30/32/35m/s. 최대 0.65초, 재사용 10초. 무적 부여 없이 충돌 검사 |
| 배치 | 기존 / A 건물 간격 / B 수직 차단벽. 설정에서 다음 게임 배치 선택. A/B는 기존 최고 기록 제외 |
| 제외 | 자동 앵커 네트워크, 마우스 두 와이어 독립 모드, 와이어 개수 설정 |
...
- `tests/launch_routes.gd`: **30/30**. E 입력 큐·방향·무적 부재·얇은 장애물 충돌·방어구·쿨다운·퍽·점 정리·양쪽 건물 조건·준비 HUD·청크 보호·원점 이동, 배치 선택 버튼·저장·다음 게임 적용·차단벽 높이.
...
- 소스 렌더링 fixture: E 두 와이어, A 건물 공백, B 수직 차단벽 화면 확인.
- `validation/compare_routes.gd`: **51회** 자동 주행. 배치만 비교 시 A/B 한쪽 고정 각각 0/6 통과, 좌우 전환 각각 3/3 통과. 세부 조건·실전 장애물·고층·E 조합은 [보고서](ROUTE_COMPARISON.md) 참고.
- EXE SHA256: `287E69046A3B0F03234F3E287F60BD837BC24EEFA61946E61541A9BD6D1FAE83`.
